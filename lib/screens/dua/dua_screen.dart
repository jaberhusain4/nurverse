...
class _DuaScreenState extends State<DuaScreen> {
  ...
  String _title(AppLocalizations l10n, DuaCategory category) {
    return l10n.isBangla ? category.titleBn : category.titleEn;
  }

  String _subtitle(AppLocalizations l10n, DuaCategory category) {
    return l10n.isBangla ? category.subtitleBn : category.subtitleEn;
  }
  ...
  List<DuaCategory> _filtered() {
    ...
    return duaCategories.where((category) {
      final categoryText =
          '${category.titleBn} ${category.titleEn} '
          '${category.subtitleBn} ${category.subtitleEn}'.toLowerCase();
      final itemText = category.items
          .map(
            (item) =>
                '${item.titleBn} ${item.titleEn} '
                '${item.translationBn} ${item.translationEn}',
          )
          .join(' ')
          .toLowerCase();
...
class _DuaCategoryScreenState extends State<DuaCategoryScreen> {
    ...
    return Scaffold(
      ...
      body: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return _buildDuaCard(context, item, l10n, primary);
        },
      ),
    );
  }
  ...
  Widget _buildDuaCard(
    ...
    final theme = Theme.of(context);
    final translation = l10n.isBangla
        ? item.translationBn
        : item.translationEn;
...
class _DuaAudioButtonState extends State<DuaAudioButton> {
  ...
  Future<void> _handleTap() async {
    ...
    try {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                path == null
                    ? l10n.tr(
                        'রেকর্ডিং সংরক্ষণ করা যায়নি।',
                        'Recording could not be saved.',
                      )
                    : l10n.tr(
                        'রেকর্ডিং সংরক্ষণ হয়েছে।',
                        'Recording saved.',
                      ),
              ),
            ),
      ...
      } else if (DuaAudioService.hasAudio(widget.item)) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                started
                    ? l10n.tr(
                        'রেকর্ডিং শুরু হয়েছে। দুআ পড়ে আবার এই বাটনে চাপুন।',
                        'Recording started. Recite the Dua, then press this button again to stop.',
                      )
                    : l10n.tr(
  ...
  Widget build(BuildContext context) {
    ...
    final color = recording ? Colors.redAccent : widget.color;
