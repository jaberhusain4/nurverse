...
String _arabicBookName(String key) {
...
class _TodayHadithCardState extends State<_TodayHadithCard> {
  ...
  Widget _buildContent(Color secondary){if(_isLoading)return Padding(padding:const EdgeInsets.symmetric(vertical:18),child:Center(child:Column(mainAxisSize:MainAxisSize.min,children:[const SizedBox(width:22,height:22,child:CircularProgressIndicator(strokeWidth:2.1)),const SizedBox(height:9),Text(widget.l10n.isArabic ? 'جارٍ تحميل الحديث...' : widget.l10n.tr('হাদিস লোড হচ্ছে...','Loading hadith...'),style:const TextStyle(fontSize:12))])));if(_error!=null||_hadith==null)return Column(children:[Icon(Icons.menu_book_outlined,size:31,color:secondary.withValues(alpha:.55)),const SizedBox(height:7),Text(_error??widget.l10n.tr('আজকের হাদিস পাওয়া যায়নি।','Today’s hadith is unavailable.'),textAlign:TextAlign.center,style:TextStyle(fontSize:13,height:1.5,color:secondary)),const SizedBox(height:8),TextButton.icon(onPressed:_load,icon:const Icon(Icons.refresh_rounded,size:17),label:Text(widget.l10n.retry))]);final hadith=_hadith!;final text=widget.l10n.isArabic ? hadith.arabic.trim() : !widget.l10n.isBangla && !widget.l10n.isArabic ? hadith.english.trim() : hadith.bangla.trim();if(text.isEmpty)return Text(widget.l10n.tr('আজকের হাদিস পাওয়া যায়নি।','Today’s hadith is unavailable.'),textAlign:TextAlign.center,style:TextStyle(color:secondary,fontSize:13));return Container(width:double.infinity,padding:const EdgeInsets.fromLTRB(15,15,15,13),decoration:BoxDecoration(color:AppColors.seaBlue.withValues(alpha:.045),borderRadius:BorderRadius.circular(16),border:Border.all(color:AppColors.seaBlue.withValues(alpha:.075))),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Icon(Icons.format_quote_rounded,color:AppColors.seaBlue,size:23),const SizedBox(height:4),Text(text,textAlign:widget.l10n.isArabic?TextAlign.right:TextAlign.start,style:const TextStyle(fontSize:15,height:1.75,fontWeight:FontWeight.w500)),const SizedBox(height:12),Container(height:1,color:AppColors.seaBlue.withValues(alpha:.09)),const SizedBox(height:9),Row(crossAxisAlignment:CrossAxisAlignment.start,children:[const Icon(Icons.menu_book_rounded,size:14,color:AppColors.seaBlue),const SizedBox(width:6),Expanded(child:Text(hadith.reference.isNotEmpty?'${widget.l10n.tr('রেফারেন্স','Reference')}: ${hadith.reference}':'${widget.l10n.tr('হাদিস','Hadith')}: ${hadith.hadithNo}',style:TextStyle(color:secondary,fontSize:10.8,height:1.45,fontWeight:FontWeight.w600)))])]));}
}
...
class _HadithCollectionCard extends StatelessWidget {
  ...
  @override Widget build(BuildContext context){final secondary=context.secondaryTextColor;final primaryName = l10n.isArabic ? _arabicBookName(book.key) : l10n.isBangla ? book.nameBn : book.nameEn;final secondaryName = l10n.isArabic ? book.nameEn : l10n.isBangla ? book.nameEn : book.nameBn;return NvCard(padding:EdgeInsets.zero,onTap:onTap,child:Padding(padding:const EdgeInsets.fromLTRB(13,12,11,12),child:Row(children:[Container(width:48,height:48,decoration:BoxDecoration(color:AppColors.seaBlue.withValues(alpha:.1),borderRadius:BorderRadius.circular(15)),alignment:Alignment.center,child:const Icon(Icons.menu_book_rounded,color:AppColors.seaBlue,size:22)),const SizedBox(width:12),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(primaryName,style:const TextStyle(fontSize:15,fontWeight:FontWeight.w800,height:1.35)),const SizedBox(height:3),Text(secondaryName,style:TextStyle(color:secondary,fontSize:11.2,height:1.4))])),Container(width:33,height:33,decoration:BoxDecoration(color:AppColors.seaBlue.withValues(alpha:.07),shape:BoxShape.circle),child:const Icon(Icons.arrow_forward_ios_rounded,size:13,color:AppColors.seaBlue))])));}
}
