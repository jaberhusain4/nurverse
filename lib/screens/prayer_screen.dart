...
class _PrayerScreenState extends State<PrayerScreen> {
  ...
  String _prayerName(AppLocalizations l10n, String value, String languageCode) {
    ...
    for (final item in names.values) {
      if (item.contains(value))
        return _label(languageCode, item[0], item[1], item[2]);
    }
  ...
  String _status(AppLocalizations l10n, String value, String languageCode) {
    const translations = <String, List<String>>{
      'ফজরের সময় শুরু হতে চলেছে': [
        'ফজরের সময় শুরু হতে চলেছে',
        'Fajr time is about to begin',
        'سيبدأ وقت الفجر قريبًا',
      ],
      'ফজরের ওয়াক্ত চলছে': [
        'ফজরের ওয়াক্ত চলছে',
        'Fajr time is active',
      ...
      'পরবর্তী সালাত: জুমু‘আ': [
        'পরবর্তী সালাত: জুমু‘আ',
        'Next prayer: Jumu’ah',
        'الصلاة التالية: الجمعة',
      ],
      'পরবর্তী সালাত: যোহর': [
        'পরবর্তী সালাত: যোহর',
        'Next prayer: Dhuhr',
        'الصلاة التالية: الظهر',
      ],
      'জুমু‘আর ওয়াক্ত চলছে': [
        'জুমু‘আর ওয়াক্ত চলছে',
        'Jumu’ah time is active',
        'وقت الجمعة جارٍ',
      ],
      'যোহরের ওয়াক্ত চলছে': [
        'যোহরের ওয়াক্ত চলছে',
        'Dhuhr time is active',
        'وقت الظهر جارٍ',
      ],
      'আসরের ওয়াক্ত চলছে': [
        'আসরের ওয়াক্ত চলছে',
  ...
  Widget _prayerRow(
    ...
    final start = _displayTime(
      data['start']?.toString() ?? '--:--',
      showSeconds,
    );
    final end = _displayTime(data['end']?.toString() ?? '--:--', showSeconds);
    final jamaat = _displayTime(
      data['jamaat']?.toString() ?? '--:--',
      showSeconds,
    );
        ...
        Padding(
          ...
          child: Row(
            children: [
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 9),
                        Icon(Icons.stop_rounded, size: 16, color: secondary),
                        const SizedBox(width: 3),
                        Flexible(
                          child: FittedBox(
                            fit: BoxFit.scaleDown,
              ...
              ConstrainedBox(
                constraints: const BoxConstraints(minWidth: 76, maxWidth: 98),
                child: Column(
  ...
  Widget _importantTimes(
    BuildContext context,
    AppLocalizations l10n,
    PrayerController controller,
    Color primary,
    SunTimeInfo? sunTimes,
  ) {
    final showSeconds = context.read<SettingsProvider>().showSeconds;
    return Row(
      children: [
        ...
        Expanded(
          child: _infoMiniCard(
            ...
            l10n.solarNoon,
            _displayTime(controller.solarNoonTime, showSeconds),
            primary,
          ),
        ),
        const SizedBox(width: 7),
      ...
      child: Column(
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              color: text.withValues(alpha: .78),
              fontSize: 12,
      ...
      child: Column(
        children: [
          for (var i = 0; i < nafl.length; i++) ...[
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: Icon(
                      _prayerIcon(nafl[i]['name']?.toString() ?? ''),
                      size: 21,
                      color: primary,
                    ),
                  ),
                  ...
                  Flexible(
                    child: FittedBox(
                      ...
                      alignment: Alignment.centerRight,
                      child: Text(
                        '${_displayTime(nafl[i]['start']?.toString() ?? '--:--', showSeconds)} – ${_displayTime(nafl[i]['end']?.toString() ?? '--:--', showSeconds)}',
                        maxLines: 1,
                        style: TextStyle(
                          color: primary,
                          fontSize: 12.5,
            ...
            if (i < nafl.length - 1)
              Divider(height: 1, color: secondary.withValues(alpha: .10)),
          ],
        ],
      ),
    );
  ...
  Widget _trackerCard(String languageCode) =>
      _DailyPrayerTrackerCard(languageCode: languageCode);
...
class _DailyPrayerTrackerCard extends StatefulWidget {
  ...
  @override
  State<_DailyPrayerTrackerCard> createState() =>
      _DailyPrayerTrackerCardState();
}
...
class _DailyPrayerTrackerCardState extends State<_DailyPrayerTrackerCard> {
  ...
  Future<Map<String, bool>> _readDay(
    ...
    for (final prayer in _prayers) {
      result[prayer] =
          prefs.getBool('nurverse_tracker_${key}_$prayer') ?? false;
    }
    return result;
  }
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: Text(
                  _label('সালাত ট্র্যাকার', 'Prayer tracker', 'متابعة الصلاة'),
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(
                    color: text,
          ...
          Align(
            ...
            child: Text(
              _loading
                  ? _label('লোড হচ্ছে...', 'Loading...', 'جار التحميل...')
                  : '${_dateText(now)} • ${_label('আজ', 'Today', 'اليوم')}',
              style: TextStyle(
                color: secondary,
                fontSize: 11.5,
                fontWeight: FontWeight.w600,
          ...
          Row(
            children: [
              for (var i = 0; i < _prayers.length; i++) ...[
                Expanded(
                  child: InkWell(
                    onTap: _loading ? null : () => _toggle(_prayers[i]),
                    borderRadius: BorderRadius.circular(12),
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          Icon(
                            (_today[_prayers[i]] ?? false)
                                ? Icons.check_circle_rounded
                                : Icons.radio_button_unchecked_rounded,
                            color: (_today[_prayers[i]] ?? false)
                                ? primary
                                : secondary.withValues(alpha: .65),
                            size: 23,
                          ),
                          const SizedBox(height: 4),
                          FittedBox(
          ...
          Container(
            ...
            child: Row(
              children: [
                Expanded(
                  child: Text(
                    '${_label('আজ', 'Today', 'اليوم')}: $todayCount/5',
                    style: TextStyle(
                      color: text,
                      fontSize: 11.5,
                      fontWeight: FontWeight.w700,
                ...
                Expanded(
                  child: Text(
                    '${_label('গতকাল', 'Yesterday', 'أمس')}: $yesterdayCount/5',
          ...
          Align(
            alignment: Alignment.centerLeft,
            child: Text(
              '${_dateText(now.subtract(const Duration(days: 1)))} • ${yesterdayCount == 5 ? _label('সব ৫ ওয়াক্ত আদায় হয়েছে', 'All 5 prayers completed', 'أُديت الصلوات الخمس') : _label('$yesterdayCount ওয়াক্ত আদায় হয়েছে', '$yesterdayCount prayers completed', 'تم أداء $yesterdayCount صلوات')}',
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              style: TextStyle(
                color: secondary,
