...
class _AudioQuranScreenState extends State<AudioQuranScreen> {
  ...
  Future<void> _togglePlay() async {
    try {
      ...
      // sequenceState itself can be null.
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('অডিও চালু করা যায়নি: $e')));
    }
  }
  ...
  Future<void> _downloadCurrent() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('ডাউনলোড করা যায়নি: $e')));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          _buildPlayer(context, current),
          Expanded(child: _buildSurahList(context, surahs)),
        ],
      ),
    );
  }
  ...
  Widget _buildPlayer(BuildContext context, QuranSurah? current) {
    ...
    final title = current?.banglaName ?? current?.transliteration ?? 'সূরা';
        ...
        child: Column(
          children: [
            Row(
              children: [
                Container(
                  ...
                  child: Icon(Icons.headphones_rounded, color: primary),
                ),
                ...
                PopupMenuButton<QuranReciter>(
                  tooltip: 'ক্বারী নির্বাচন',
                  onSelected: _changeReciter,
                  itemBuilder:
                      (_) => [
                        for (final reciter in kReciters)
                          PopupMenuItem<QuranReciter>(
                            value: reciter,
                            child: Text(reciter.nameBn),
                          ),
                      ],
            ...
            Row(
              ...
              children: [
                IconButton(
                  onPressed: _selectedSurah > 1 ? _previous : null,
                ...
                IconButton(
                  onPressed: _selectedSurah < 114 ? _next : null,
