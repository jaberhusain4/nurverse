...
class _OnudhabonQuranReaderState extends State<OnudhabonQuranReader> {
  ...
  Widget _buildReader(BuildContext context, QuranSurah surah) {
    ...
    return Column(
      children: [
        ...
        if (_tafsirError != null) _error(context, _localizedText(l10n, 'তাফসির লোড করা যায়নি।', 'Could not load Tafsir.', 'تعذر تحميل التفسير.')),
        Expanded(
          child: Builder(
            builder: (context) {
              final primary = Theme.of(context).colorScheme.primary;
  ...
  Widget _surahHeader(BuildContext context, QuranSurah surah) {
    ...
    final banglaMeaning = _surahBanglaMeaning(surah.number);
    return Container(
          ...
          Expanded(
            child: Column(
              children: [
                ...
                if (l10n.isBangla && banglaMeaning != null) ...[
                  const SizedBox(height: 2),
                  Text('অর্থ: $banglaMeaning', style: TextStyle(fontSize: 11.5, fontWeight: FontWeight.w500, color: context.secondaryTextColor)),
                ],
              ],
            ),
          ),
