...
class SettingsHubScreenV4 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(isEnglish ? 'Settings' : 'সেটিংস'),
      ),
      body: ListView(
            ...
            [
              ...
              _switchTile(
                ...
                isEnglish
                    ...
                    : 'সমর্থিত action-এ haptic feedback',
      ...
      child: InkWell(
        onTap: () async {
          if (premium.isPremium) {
            await _showPremiumStatus(context, premium, isEnglish);
          } else {
                  ...
                  children: [
                    Text(
                      premium.isPremium
                          ? 'NurVerse Premium'
                          : (isEnglish
                              ? 'Unlock NurVerse Premium'
                              : 'NurVerse Premium চালু করুন'),
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          TextButton(
            ...
            child: Text(isEnglish ? 'Done' : 'ঠিক আছে'),
          ),
        ],
      ),
    );
              ...
              children: [
                for (final prayer in prayers)
                  ListTile(
                    title: Text(prayer),
                    subtitle: Text(
                      '${settings.prayerAdjustments[prayer] ?? 0} min',
                    ),
                    trailing: Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
    ...
    try {
      await showDialog<void>(
        ...
        builder: (dialogContext) {
          return AlertDialog(
            title: Text(
              settings.isEnglish ? 'Jamaat Times' : 'জামাতের সময়',
            ),
            content: SingleChildScrollView(
              child: Column(
                children: [
  ...
  Future<void> _showDailyContentSheet(
    BuildContext context,
    SettingsProvider settings,
  ) async {
    await showModalBottomSheet<void>(
