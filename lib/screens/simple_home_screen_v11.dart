          ...
          child: ListView(
            ...
            children: [
              ...
              _Hero(
                now: _now,
                clock: _clock(),
                nextPrayer: c.nextPrayerName,
                nextTime: c.nextPrayerTime,
                remaining: c.timeRemainingForNextPrayer,
                currentPrayer: c.currentPrayer,
                progress: c.prayerProgress,
                sunrise: c.sunriseTime,
                sunset: c.sunsetTime,
                motion: _motion,
                lang: lang,
              ),
              ...
              if (hasLastRead) ...[
                ...
                ContinueReadingCard(
                  ...
                  paraNo: int.tryParse('${_lastRead!['paraNo']}') ?? 1,
                  pageNo: int.tryParse('${_lastRead!['pageNo']}') ?? 1,
              ...
              _Footer(
                ...
                countdown: _countdown(prohibited ? c.prohibitedEnd : c.prohibitedStart),
                lang: lang,
              ),
            ],
          ),
    ...
    return Row(
      ...
      children: [
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(greeting, style: Theme.of(context).textTheme.titleLarge?.copyWith(fontSize: 21, fontWeight: FontWeight.w800, color: primary, height: 1.05)),
          const SizedBox(height: 7),
          Row(children: [Icon(Icons.location_on_outlined, size: 15, color: context.secondaryTextColor), const SizedBox(width: 4), Expanded(child: Text(city, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 12.5, color: context.secondaryTextColor)))])
        ])),
        const SizedBox(width: 10),
        Container(
          constraints: const BoxConstraints(maxWidth: 145),
          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 8),
          decoration: BoxDecoration(color: primary.withValues(alpha: .07), borderRadius: BorderRadius.circular(14)),
          child: Text(hijri, textAlign: TextAlign.end, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: context.primaryTextColor)),
        )
      ],
    );
  }
...
class _Hero extends StatelessWidget {
  ...
  final DateTime now;
  final String clock, nextPrayer, nextTime, remaining, currentPrayer, sunrise, sunset, lang;
  ...
  Widget build(BuildContext context) {
    ...
    final night = hour < 5.0 || hour >= 19.5;
    final dawn = hour >= 5.0 && hour < 7.0;
    final dusk = hour >= 16.5 && hour < 19.5;
    final sea = Theme.of(context).colorScheme.primary;
    final palette = _ScenePalette.forTime(hour, sea);
    ...
    return AnimatedBuilder(
      ...
      builder: (context, _) => Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: .16), blurRadius: 28, offset: const Offset(0, 14))],
        ),
        child: Stack(children: [
          Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [palette.top, palette.mid, palette.bottom])))),
          Positioned.fill(child: CustomPaint(painter: _DesertPainter(palette: palette, t: motion.value, night: night, dawn: dawn, dusk: dusk))),
          Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Colors.transparent, Colors.black.withValues(alpha: .03), Colors.black.withValues(alpha: .38)])))),
          Padding(
            padding: const EdgeInsets.fromLTRB(20, 18, 20, 17),
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Text(clock, style: Theme.of(context).textTheme.titleMedium?.copyWith(color: Colors.white.withValues(alpha: .95), fontSize: 16, fontWeight: FontWeight.w600, letterSpacing: .3, fontFeatures: const [FontFeature.tabularFigures()])),
                const Spacer(),
                _SceneBadge(text: night ? (lang == 'en' ? 'NIGHT' : 'রাত') : dusk ? (lang == 'en' ? 'GOLDEN HOUR' : 'সন্ধ্যা') : (lang == 'en' ? 'DAYLIGHT' : 'দিন')),
              ]),
              const Spacer(),
              Text(nextPrayer.isEmpty ? '--' : nextPrayer, maxLines: 1, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.headlineSmall?.copyWith(color: Colors.white, fontSize: 30, fontWeight: FontWeight.w800, height: 1.0)),
              const SizedBox(height: 4),
              Text(nextTime.isEmpty ? '--:--' : nextTime, style: TextStyle(color: Colors.white.withValues(alpha: .84), fontSize: 13.5, fontWeight: FontWeight.w600)),
              const SizedBox(height: 9),
              Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Expanded(child: Text(remaining.isEmpty ? '--:--:--' : remaining, style: Theme.of(context).textTheme.displaySmall?.copyWith(color: Colors.white, fontSize: 35, fontWeight: FontWeight.w500, height: .95, letterSpacing: -.8, fontFeatures: const [FontFeature.tabularFigures()]))),
                if (currentPrayer.isNotEmpty) Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text(lang == 'en' ? 'NOW' : 'এখন', style: TextStyle(color: Colors.white.withValues(alpha: .55), fontSize: 9, fontWeight: FontWeight.w700, letterSpacing: 1)), const SizedBox(height: 2), Text(currentPrayer, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w700))])
              ]),
              const SizedBox(height: 11),
              _DayArc(value: progress.clamp(0.0, 1.0).toDouble(), accent: sea.withValues(alpha: .95)),
              const SizedBox(height: 6),
              Row(children: [
                _SunMeta(icon: Icons.wb_sunny_outlined, label: lang == 'en' ? 'Sunrise' : 'সূর্যোদয়', value: sunrise),
                const Spacer(),
                _SunMeta(icon: Icons.nightlight_round, label: lang == 'en' ? 'Sunset' : 'সূর্যাস্ত', value: sunset, end: true),
              ]),
            ]),
          )
        ]),
...
class _SceneBadge extends StatelessWidget {
  const _SceneBadge({required this.text});
  final String text;
  @override
...
class _SunMeta extends StatelessWidget {
  const _SunMeta({required this.icon, required this.label, required this.value, this.end = false});
  final IconData icon; final String label, value; final bool end;
  @override
...
class _ScenePalette {
  const _ScenePalette(this.top, this.mid, this.bottom, this.sun, this.sandFar, this.sandNear, this.sea);
  final Color top, mid, bottom, sun, sandFar, sandNear, sea;
  static _ScenePalette forTime(double h, Color sea) {
    if (h < 5 || h >= 19.5) return _ScenePalette(const Color(0xFF061B2C), const Color(0xFF0D3A57), const Color(0xFF103F59), const Color(0xFFBDEBFF), const Color(0xFF173B4A), const Color(0xFF071F2C), sea);
    if (h < 7) return _ScenePalette(const Color(0xFF155B78), const Color(0xFF3C9CB0), const Color(0xFFE3B07B), const Color(0xFFFFE0A4), const Color(0xFFB47757), const Color(0xFF513A35), sea);
    if (h >= 16.5) return _ScenePalette(const Color(0xFF176B91), const Color(0xFF5BA6B7), const Color(0xFFD8896D), const Color(0xFFFFD08A), const Color(0xFF9B6657), const Color(0xFF493B39), sea);
    return _ScenePalette(const Color(0xFF14729B), const Color(0xFF51B8D1), const Color(0xFFB7D5C8), const Color(0xFFFFF1BD), const Color(0xFFC59A68), const Color(0xFF66503F), sea);
  }
}
...
class _DesertPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final sun = Paint()..color = palette.sun.withValues(alpha: night ? .20 : .95);
    canvas.drawCircle(sunPos, night ? 27 : 31, Paint()..color = palette.sun.withValues(alpha: .10));
    canvas.drawCircle(sunPos, night ? 17 : 20, sun);
    if (night) {
      final star = Paint()..color = Colors.white.withValues(alpha: .45);
      for (var i = 0; i < 24; i++) {
        final x = ((i * 83) % 100) / 100 * w;
        final y = ((i * 47) % 48) / 100 * h;
        canvas.drawCircle(Offset(x, y), i % 4 == 0 ? 1.5 : .8, star);
      }
    }

    _cloud(canvas, Offset(w * (.17 + .06 * t), h * .20), 1.0, night ? .10 : .28);
    _cloud(canvas, Offset(w * (.53 - .08 * t), h * .29), .72, night ? .06 : .20);
    _cloud(canvas, Offset(w * (.78 + .04 * t), h * .14), .52, night ? .04 : .15);

    _dune(canvas, w, h, h * .68, palette.sandFar, .10, 0.0);
    _dune(canvas, w, h, h * .76, palette.sandFar.withValues(alpha: .82), .13, .35);
    _dune(canvas, w, h, h * .84, palette.sandNear, .16, .62);

    final oasis = Offset(w * .66, h * .79);
    canvas.drawOval(Rect.fromCenter(center: oasis, width: w * .27, height: h * .055), Paint()..color = const Color(0xFF4FA5A1).withValues(alpha: .75));
    ...
    _palm(canvas, Offset(w * .18, h * .68), .88, palette.sea.withValues(alpha: .9));
    _palm(canvas, Offset(w * .83, h * .69), .68, palette.sea.withValues(alpha: .72));
    _palm(canvas, Offset(w * .59, h * .72), .54, palette.sea.withValues(alpha: .65));
    _camel(canvas, Offset(w * (.38 + .025 * sin(t * 6.283)), h * .77), .85, palette.sea.withValues(alpha: .92));
  }

  void _cloud(Canvas canvas, Offset c, double scale, double opacity) {
    final p = Paint()..color = Colors.white.withValues(alpha: opacity);
    canvas.drawCircle(c.translate(-24 * scale, 3), 18 * scale, p);
    canvas.drawCircle(c.translate(0, -7), 24 * scale, p);
    canvas.drawCircle(c.translate(23 * scale, 3), 17 * scale, p);
    canvas.drawOval(Rect.fromCenter(center: c.translate(0, 9), width: 70 * scale, height: 25 * scale), p);
    final soft = Paint()..color = Colors.white.withValues(alpha: opacity * .25)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 8);
    canvas.drawOval(Rect.fromCenter(center: c.translate(0, 11), width: 76 * scale, height: 20 * scale), soft);
  }

  void _dune(Canvas canvas, double w, double h, double y, Color color, double curve, double phase) {
    final p = Path()..moveTo(0, y);
    p.quadraticBezierTo(w * .22, y - h * curve, w * .46, y + h * curve * .35);
    p.quadraticBezierTo(w * .72, y + h * curve, w, y - h * curve * .18);
    p.lineTo(w, h); p.lineTo(0, h); p.close();
    canvas.drawPath(p, Paint()..color = color);
  }

  void _palm(Canvas canvas, Offset base, double s, Color color) {
    final trunk = Paint()..color = color.withValues(alpha: .9)..strokeWidth = 4.5 * s..strokeCap = StrokeCap.round;
    canvas.drawLine(base, base.translate(4 * s, -55 * s), trunk);
    final leaf = Paint()..color = color;
    final top = base.translate(4 * s, -55 * s);
    for (final a in [-1.25, -.85, -.42, 0.15, .55, .95, 1.35]) {
      final end = top + Offset(cos(a) * 25 * s, sin(a) * 17 * s - 7 * s);
      final path = Path()..moveTo(top.dx, top.dy)..quadraticBezierTo((top.dx + end.dx) / 2, top.dy - 7 * s, end.dx, end.dy);
      canvas.drawPath(path, leaf..style = PaintingStyle.stroke..strokeWidth = 2.5 * s..strokeCap = StrokeCap.round);
    }
  }
  ...
  void _camel(Canvas canvas, Offset base, double s, Color color) {
    ...
    canvas.drawCircle(base.translate(-5 * s, -19 * s), 7 * s, Paint()..color = color.withValues(alpha: .95));
  }
...
class _DayArc extends StatelessWidget {
  const _DayArc({required this.value, required this.accent});
  final double value; final Color accent;
  @override
  Widget build(BuildContext context) => SizedBox(height: 42, width: double.infinity, child: CustomPaint(painter: _ArcPainter(value, accent)));
}
...
class _ArcPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final fg = Paint()..color = accent..style = PaintingStyle.stroke..strokeWidth = 5..strokeCap = StrokeCap.round;
    canvas.drawArc(rect, pi, pi * value.clamp(0, 1), false, fg);
    final a = pi + pi * value.clamp(0, 1);
    final center = Offset(size.width / 2, 4);
    final rx = (size.width - 16) / 2, ry = size.height * .95;
    final dot = Offset(center.dx + rx * cos(a), center.dy + ry * sin(a));
    canvas.drawCircle(dot, 9, Paint()..color = accent.withValues(alpha: .22));
...
class _Section extends StatelessWidget {
  const _Section({required this.title, required this.subtitle}); final String title, subtitle;
...
class _Essentials extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) {
    final primary = Theme.of(context).colorScheme.primary;
    final items = <_Item>[
      _Item(Icons.explore_rounded, lang == 'en' ? 'Qibla' : 'কিবলা', onQibla), _Item(Icons.favorite_rounded, lang == 'en' ? 'Dua' : 'দোয়া', onDua), _Item(Icons.touch_app_rounded, lang == 'en' ? 'Tasbih' : 'তাসবিহ', onTasbih), _Item(Icons.auto_awesome_rounded, lang == 'en' ? '99 Names' : '৯৯ নাম', onNames), _Item(Icons.calendar_month_rounded, lang == 'en' ? 'Calendar' : 'ক্যালেন্ডার', onCalendar), _Item(Icons.health_and_safety_rounded, lang == 'en' ? 'Ruqyah' : 'রুকইয়াহ', onRuqyah)];
