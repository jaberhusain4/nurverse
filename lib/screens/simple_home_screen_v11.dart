...
class _SimpleHomeScreenV11State extends State<SimpleHomeScreenV11> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    _motion = AnimationController(vsync: this, duration: const Duration(seconds: 24))..repeat(reverse: true);
      ...
      body: SafeArea(
        ...
        child: RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              _Header(greeting: _greeting(code), location: controller.currentLocationName, hijri: _hijri(code)),
              ...
              _SectionTitle(title: _tr('আজকের সালাত', 'Today’s Prayer', code), subtitle: _tr('পাঁচ ওয়াক্ত, এক নজরে', 'Five prayers at a glance', code)),
              const SizedBox(height: 10),
              _PrayerTimeline(prayers: prayers, code: code, showSeconds: settings.showSeconds),
              const SizedBox(height: 22),
              _SectionTitle(title: _tr('প্রয়োজনীয়', 'Essentials', code), subtitle: _tr('প্রতিদিনের গুরুত্বপূর্ণ সুবিধা', 'Everyday essentials', code)),
              const SizedBox(height: 10),
              _Essentials(code: code, onNavigateTab: widget.onNavigateTab),
              const SizedBox(height: 22),
              _Footer(date: DateService.englishDate(), code: code, prohibited: controller.prohibitedTimeText.toString()),
            ],
          ),
        ),
      ),
...
class _Header extends StatelessWidget {
  ...
  final String greeting, location, hijri;
  @override
  Widget build(BuildContext context) {
    ...
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [Icon(Icons.location_on_outlined, size: 15, color: secondary), const SizedBox(width: 4), Expanded(child: Text(location.trim().isEmpty ? 'Locating…' : location.trim(), maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 12, color: secondary)))])
      ])),
      const SizedBox(width: 12),
      Flexible(child: Text(hijri, textAlign: TextAlign.end, maxLines: 2, overflow: TextOverflow.ellipsis, style: theme.textTheme.labelMedium?.copyWith(fontSize: 10.5, fontWeight: FontWeight.w700)))
    ]);
  }
}

class _UnifiedPrayerHero extends StatelessWidget {
  const _UnifiedPrayerHero({required this.now, required this.motion, required this.clock, required this.nextPrayer, required this.nextPrayerTime, required this.remaining, required this.currentPrayer, required this.progress, required this.sunrise, required this.sunset});
  final DateTime now;
  final double motion;
  final String clock, nextPrayer, nextPrayerTime, remaining, currentPrayer, sunrise, sunset;
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(30), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: .18), blurRadius: 30, offset: const Offset(0, 16))]),
      child: Stack(fit: StackFit.expand, children: [
        CustomPaint(painter: _AtmosphericPainter(palette: palette, motion: motion)),
        Positioned.fill(child: DecoratedBox(decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Colors.black.withValues(alpha: .02), Colors.black.withValues(alpha: .05), Colors.black.withValues(alpha: .36)], stops: const [0, .46, 1])))),
        Positioned(left: 20, right: 20, top: 18, child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Expanded(child: Text(clock, style: const TextStyle(color: Colors.white, fontSize: 31, fontWeight: FontWeight.w700, letterSpacing: 1.2, height: 1))), _GlassPill(label: nextPrayer, value: nextPrayerTime)])),
        Positioned(left: 20, right: 20, bottom: 20, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          SizedBox(height: 48, child: CustomPaint(painter: _DayArcPainter(progress: progress, accent: primary), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, crossAxisAlignment: CrossAxisAlignment.end, children: [_TinyTime(label: 'SUNRISE', value: sunrise), _TinyTime(label: 'SUNSET', value: sunset)])))
        ]))
      ]),
    );
  }
...
class _GlassPill extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text(label, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w800)), const SizedBox(height: 2), Text(value, style: TextStyle(color: Colors.white.withValues(alpha: .72), fontSize: 10, fontWeight: FontWeight.w600))]),
  );
}
...
class _TinyTime extends StatelessWidget {
  ...
  final String label, value;
  @override
  Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(label, style: TextStyle(color: Colors.white.withValues(alpha: .55), fontSize: 8, fontWeight: FontWeight.w700, letterSpacing: 1.2)), const SizedBox(height: 2), Text(value, style: const TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700))]);
}

class _Atmosphere {
  const _Atmosphere({required this.top, required this.bottom, required this.horizon, required this.night});
  final Color top, bottom, horizon;
  final bool night;
...
class _AtmosphericPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = 0; x <= size.width; x += 4) farPath.lineTo(x, size.height * .60 + math.sin(x / 65 + motion * 1.2) * 9 + math.sin(x / 31) * 4);
    farPath..lineTo(size.width, size.height)..lineTo(0, size.height)..close();
    ...
    for (double x = 0; x <= size.width; x += 4) dune.lineTo(x, size.height * .67 + math.sin(x / 105 + motion * .8) * 18 + math.sin(x / 45) * 5);
    dune..lineTo(size.width, size.height)..lineTo(0, size.height)..close();
    ...
    if (palette.night) {
      ...
      for (int i = 0; i < 22; i++) { final x = (i * 83.0) % size.width; final y = 24 + ((i * 37.0) % (size.height * .40)); canvas.drawCircle(Offset(x, y), i % 3 == 0 ? 1.4 : .8, star); }
    }
  }
  ...
  void _palm(Canvas canvas, Size size, Offset base, double scale) {
    ...
    for (int i = 0; i < 7; i++) { final a = -math.pi + i * math.pi / 6; final end = Offset(base.dx + math.cos(a) * 38 * scale, base.dy - 90 * scale + math.sin(a) * 22 * scale); canvas.drawLine(Offset(base.dx, base.dy - 90 * scale), end, leaf..strokeWidth = 5 * scale..strokeCap = StrokeCap.round); }
  }
  ...
  @override bool shouldRepaint(covariant _AtmosphericPainter oldDelegate) => oldDelegate.motion != motion || oldDelegate.palette != palette;
}
...
class _DayArcPainter extends CustomPainter {
  ...
  @override bool shouldRepaint(covariant _DayArcPainter oldDelegate) => oldDelegate.progress != progress || oldDelegate.accent != accent;
}
...
class _Essentials extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final primary = Theme.of(context).colorScheme.primary;
    return GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: 6, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 3, crossAxisSpacing: 9, mainAxisSpacing: 9, childAspectRatio: 1.05), itemBuilder: (_, i) => Material(color: primary.withValues(alpha: .075), borderRadius: BorderRadius.circular(18), child: InkWell(borderRadius: BorderRadius.circular(18), onTap: () { if (i == 0) onNavigateTab?.call(4); }, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icons[i], size: 24, color: primary), const SizedBox(height: 7), Text(code == 'en' ? en[i] : bn[i], textAlign: TextAlign.center, style: const TextStyle(fontSize: 11.5, fontWeight: FontWeight.w700))]))));
  }
}

class _Footer extends StatelessWidget {
  const _Footer({required this.date, required this.code, required this.prohibited});
  final String date, code, prohibited;
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.symmetric(horizontal: 15, vertical: 13), decoration: BoxDecoration(color: Theme.of(context).colorScheme.primary.withValues(alpha: .055), borderRadius: BorderRadius.circular(18)), child: Row(children: [Expanded(child: Text(date, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700))), const SizedBox(width: 10), Flexible(child: Text('${code == 'en' ? 'Prohibited time' : 'নিষিদ্ধ সময়'}: $prohibited', textAlign: TextAlign.end, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 10.5, color: context.secondaryTextColor, fontWeight: FontWeight.w600)))]));
}
