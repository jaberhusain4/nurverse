...
class _IslamicBooksScreenState extends State<IslamicBooksScreen> {
  static const _readBooks = <_BookResource>[
    ...
    _BookResource(title: 'কোরআন শরীফ — প্রথম খণ্ড', description: 'মোহাম্মদ আকরম খাঁর বাংলা অনুবাদগ্রন্থ।', source: 'Wikimedia Commons • Bengali PDF', icon: Icons.library_books_rounded, url: 'https://commons.wikimedia.org/wiki/Special:Redirect/file/কোরআন_শরীফ_(প্রথম_খণ্ড)_-_মোহাম্মদ_আকরম_খাঁ.pdf', isPdf: true),
    _BookResource(title: 'কোরআন শরীফ — দ্বিতীয় খণ্ড', description: 'মোহাম্মদ আকরম খাঁর বাংলা অনুবাদগ্রন্থের দ্বিতীয় খণ্ড।', source: 'Wikimedia Commons • Public Domain', icon: Icons.library_books_rounded, url: 'https://commons.wikimedia.org/wiki/Special:Redirect/file/কোরআন_শরীফ_(দ্বিতীয়_খণ্ড)_-_মোহাম্মদ_আকরম_খাঁ.pdf', isPdf: true),
    _BookResource(title: 'মহম্মদের জীবনচরিত্র', description: 'রাসূলুল্লাহ ﷺ-এর জীবনচরিতের বাংলা ঐতিহাসিক গ্রন্থ।', source: 'Wikimedia Commons • Public Domain', icon: Icons.menu_book_rounded, url: 'https://commons.wikimedia.org/wiki/Special:Redirect/file/মহম্মদের_জীবনচরিত্র.pdf', isPdf: true),
  ];
  ...
  @override
  void initState() { super.initState(); _loadDownloadedBooks(); }
  ...
  Future<void> _downloadBook(_BookResource book) async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() { _downloadedPaths[book.title] = file.path; _downloadProgress.remove(book.title); });
  ...
  Future<void> _deleteBook(_BookResource book) async {
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (context) => AlertDialog(title: const Text('বই মুছে ফেলবেন?'), content: Text('“${book.title}” ফোনের offline storage থেকে মুছে যাবে।'), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('না')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('মুছে ফেলুন'))]));
    if (confirmed != true) return;
    try { await File(localPath).delete(); } catch (_) {}
    final prefs = await SharedPreferences.getInstance();
    await prefs.remove(_storageKey(book));
    if (mounted) { setState(() => _downloadedPaths.remove(book.title)); _showMessage('বইটি offline storage থেকে মুছে ফেলা হয়েছে।'); }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: ListView(padding: const EdgeInsets.fromLTRB(16, 8, 16, 30), children: [
        const _BooksIntroCard(), const SizedBox(height: 22),
        const _SectionHeader(title: 'Read E-Book', subtitle: 'অনলাইনে পড়ার জন্য নির্বাচিত বাংলা বই', icon: Icons.menu_book_rounded), const SizedBox(height: 10),
        ...
        const _SectionHeader(title: 'Download E-Book', subtitle: 'ডাউনলোড করে offline-এ পড়ুন', icon: Icons.download_rounded), const SizedBox(height: 10),
        ..._downloadBooks.map((book) => Padding(padding: const EdgeInsets.only(bottom: 12), child: _BookCard(book: book, actionLabel: _downloadProgress.containsKey(book.title) ? 'ডাউনলোড হচ্ছে' : _downloadedPaths.containsKey(book.title) ? 'Offline-এ পড়ুন' : 'ডাউনলোড', progress: _downloadProgress[book.title], onTap: () => _openBook(book), onDownload: _downloadedPaths.containsKey(book.title) ? () => _deleteBook(book) : () => _downloadBook(book), downloaded: _downloadedPaths.containsKey(book.title)))),
        if (downloaded.isNotEmpty) ...[const SizedBox(height: 10), const _SectionHeader(title: 'Offline Books', subtitle: 'ইন্টারনেট ছাড়াই পড়ার জন্য সংরক্ষিত বই', icon: Icons.offline_pin_rounded), const SizedBox(height: 10), ...downloaded.map((book) => Padding(padding: const EdgeInsets.only(bottom: 12), child: _BookCard(book: book, actionLabel: 'পড়ুন', onTap: () => _openBook(book), downloaded: true, onDownload: () => _deleteBook(book))))],
      ]),
    );
  }

  void _showSourceInfo(BuildContext context) {
    showModalBottomSheet<void>(context: context, showDragHandle: true, builder: (context) => Padding(padding: const EdgeInsets.fromLTRB(20, 4, 20, 28), child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [Text('বইয়ের উৎস', style: TextStyle(color: context.primaryTextColor, fontSize: 18, fontWeight: FontWeight.w800)), const SizedBox(height: 8), Text('NurVerse আপাতত শুধু বাংলা ভাষার নির্বাচিত ইসলামিক বই রাখছে। Download E-Book অংশে উন্মুক্ত লাইসেন্সের PDF রাখা হয়েছে, যাতে ডাউনলোড করে offline-এ পড়া যায়।', style: TextStyle(color: context.secondaryTextColor, fontSize: 13, height: 1.55))]));
  }
}
...
class _BooksIntroCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(18), child: Row(children: [Container(width: 48, height: 48, decoration: BoxDecoration(color: AppColors.seaBlue.withValues(alpha: 0.10), borderRadius: BorderRadius.circular(15)), child: const Icon(Icons.auto_stories_rounded, color: AppColors.seaBlue, size: 25)), const SizedBox(width: 14), Expanded(child: Text('নির্বাচিত বাংলা ইসলামিক বই — অনলাইনে পড়ুন অথবা অনুমোদিত বই offline-এ সংরক্ষণ করুন।', style: TextStyle(color: context.secondaryTextColor, fontSize: 13, height: 1.5)))])));
}
...
class _SectionHeader extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(icon, size: 20, color: AppColors.seaBlue), const SizedBox(width: 9), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(color: context.primaryTextColor, fontSize: 16, fontWeight: FontWeight.w800)), const SizedBox(height: 2), Text(subtitle, style: TextStyle(color: context.secondaryTextColor, fontSize: 12))]))]);
}
