...
class CurrentPrayerPremiumCard extends StatelessWidget {
  ...
  String _awalText() {
    ...
    final total = next.difference(start);
    if (total.inMinutes <= 0) return _label(bn: 'আওয়াল ওয়াক্তের তথ্য প্রস্তুত হচ্ছে', en: 'Awal Waqt information is being prepared', ar: 'جارٍ تجهيز وقت الأول');
    final end = start.add(Duration(milliseconds: total.inMilliseconds ~/ 3));
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(color: theme.cardColor.withValues(alpha: theme.brightness == Brightness.dark ? .72 : .88), borderRadius: BorderRadius.circular(22), border: Border.all(color: primary.withValues(alpha: .10))),
