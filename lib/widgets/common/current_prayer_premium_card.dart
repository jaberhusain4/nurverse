  ...
  Widget build(BuildContext context) {
    ...
    return Container(width: double.infinity, padding: const EdgeInsets.fromLTRB(14, 13, 14, 12), decoration: BoxDecoration(color: theme.cardColor.withValues(alpha: theme.brightness == Brightness.dark ? .72 : .88), borderRadius: BorderRadius.circular(22), border: Border.all(color: primary.withValues(alpha: .10))), child: Column(children: [
      Row(children: [
        ...
        Expanded(child: Column(children: [Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.mosque_rounded, size: 19, color: primary), const SizedBox(width: 5), Text(_label(bn: 'বর্তমান', en: 'Current', ar: 'الحالية'), style: TextStyle(color: primary.withValues(alpha: .82), fontSize: 13, fontWeight: FontWeight.w700))]), const SizedBox(height: 4), Text(currentPrayer.isEmpty ? _label(bn: 'ওয়াক্ত নেই', en: 'No prayer', ar: 'لا صلاة') : currentPrayer, textAlign: TextAlign.center, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: text, fontSize: 17, fontWeight: FontWeight.w800)), const SizedBox(height: 2), Text(currentPrayerTime.isEmpty ? '--:--' : currentPrayerTime, style: TextStyle(color: primary, fontSize: 14, fontWeight: FontWeight.w800))])),
...
class _ContextPrayer extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Column(children: [Icon(icon, size: 19, color: color), const SizedBox(height: 3), Text(label, maxLines: 1, style: TextStyle(color: color, fontSize: 12, fontWeight: FontWeight.w600)), const SizedBox(height: 2), Text(prayer, maxLines: 1, overflow: TextOverflow.ellipsis, textAlign: TextAlign.center, style: TextStyle(color: text, fontSize: 15, fontWeight: FontWeight.w800)), Text(time.isEmpty ? '--:--' : time, style: TextStyle(color: color, fontSize: 12, fontWeight: FontWeight.w700))]);
}
