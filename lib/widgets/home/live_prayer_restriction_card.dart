...
class _LivePrayerRestrictionCardState extends State<LivePrayerRestrictionCard> {
  ...
  @override Widget build(BuildContext context){
    ...
    final sunrise=_parse(c.sunriseTime), noon=_parse(c.solarNoonTime), sunset=_parse(c.sunsetTime);
    final windows=<Map<String,dynamic>>[];
    if(sunrise!=null)windows.add({'start':sunrise.subtract(const Duration(minutes:15)),'end':sunrise.add(const Duration(minutes:15)),'bn':'সূর্যোদয়ের সময়','en':'Sunrise restriction','ar':'وقت النهي عند الشروق'});
    if(noon!=null)windows.add({'start':noon.subtract(const Duration(minutes:10)),'end':noon.add(const Duration(minutes:5)),'bn':'জাওয়ালের সময়','en':'Zawal restriction','ar':'وقت النهي عند الزوال'});
    if(sunset!=null)windows.add({'start':sunset.subtract(const Duration(minutes:15)),'end':sunset.add(const Duration(minutes:15)),'bn':'সূর্যাস্তের সময়','en':'Sunset restriction','ar':'وقت النهي عند الغروب'});
    final active=windows.where((w)=>!_now.isBefore(w['start'] as DateTime)&&_now.isBefore(w['end'] as DateTime)).toList();
    final next=windows.where((w)=>_now.isBefore(w['start'] as DateTime)).toList();
    final Map<String,dynamic>? w=active.isNotEmpty?active.first:(next.isNotEmpty?next.first:null);
    if(w==null)return const SizedBox.shrink();
    final isActive=active.isNotEmpty;
    final title=_label(w['bn'] as String,w['en'] as String,w['ar'] as String);
    final message=isActive?'$title — ${_label('এখন নামাজের নিষিদ্ধ/মাকরূহ সময় চলছে','Restricted prayer time is active','وقت النهي قائم الآن')} • ${_left((w['end'] as DateTime).difference(_now))}':'$title — ${_left((w['start'] as DateTime).difference(_now))} ${_label('পর শুরু হবে','until it starts','حتى يبدأ')}';
    final t=Theme.of(context),color=t.colorScheme.error,fg=t.colorScheme.onSurface;
    return Container(width:double.infinity,padding:const EdgeInsets.symmetric(horizontal:13,vertical:11),decoration:BoxDecoration(color:color.withValues(alpha:.08),borderRadius:BorderRadius.circular(16),border:Border.all(color:color.withValues(alpha:.25))),child:Row(crossAxisAlignment:CrossAxisAlignment.start,children:[Icon(Icons.warning_amber_rounded,color:color,size:23),const SizedBox(width:10),Expanded(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[Text(_label('সতর্কতা','Prayer Time Warning','تنبيه وقت الصلاة'),style:TextStyle(fontSize:13,fontWeight:FontWeight.w800,color:color)),const SizedBox(height:3),Text(message,style:TextStyle(fontSize:13.5,fontWeight:FontWeight.w700,color:fg))]))]);
  }
}
