...
class PrayerSpecialTimesCard extends StatelessWidget {
  ...
  const PrayerSpecialTimesCard({super.key, this.languageCode = 'bn'});
  ...
  Widget build(BuildContext context) {
    ...
    final prohibited = <_Restriction>[
      if (sunrise != null) _Restriction(_label('সূর্যোদয়', 'Sunrise', 'الشروق'), sunrise, sunrise.add(const Duration(minutes: 15))),
      if (noon != null) _Restriction(_label('জাওয়াল / মধ্যাহ্ন', 'Zawal / Solar Noon', 'الزوال'), noon.subtract(const Duration(minutes: 10)), noon),
      if (sunset != null) _Restriction(_label('সূর্যাস্ত', 'Sunset', 'الغروب'), sunset.subtract(const Duration(minutes: 15)), sunset),
    ];
    final makruh = <_Restriction>[
      if (sunrise != null) _Restriction(_label('সূর্যোদয়ের আশপাশ', 'Around Sunrise', 'حول الشروق'), sunrise.subtract(const Duration(minutes: 15)), sunrise.add(const Duration(minutes: 15))),
      if (noon != null) _Restriction(_label('জাওয়ালের আশপাশ', 'Around Zawal', 'حول الزوال'), noon.subtract(const Duration(minutes: 10)), noon.add(const Duration(minutes: 5))),
      if (sunset != null) _Restriction(_label('সূর্যাস্তের আশপাশ', 'Around Sunset', 'حول الغروب'), sunset.subtract(const Duration(minutes: 15)), sunset.add(const Duration(minutes: 15))),
    ];
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [Icon(Icons.warning_amber_rounded, color: warning, size: 21), const SizedBox(width: 8), Expanded(child: Text(_label('মাকরূহ ও নিষিদ্ধ সময়', 'Makruh & Prohibited Times', 'أوقات الكراهة والنهي'), style: TextStyle(fontSize: 15.5, fontWeight: FontWeight.w800, color: text)))]),
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ...items.map((item) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(color: active ? accent.withValues(alpha: .08) : accent.withValues(alpha: .035), borderRadius: BorderRadius.circular(13), border: Border.all(color: active ? accent.withValues(alpha: .24) : accent.withValues(alpha: .07))),
          child: Row(children: [
            Icon(active ? Icons.warning_rounded : Icons.schedule_rounded, color: accent, size: 18),
            const SizedBox(width: 8),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item.name, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: text, fontSize: 12.5, fontWeight: FontWeight.w700)), const SizedBox(height: 2), Text('${_time(item.start)} – ${_time(item.end)}', style: TextStyle(color: secondary, fontSize: 11, fontWeight: FontWeight.w600))])),
            if (active) ...[const SizedBox(width: 8), Text(_remaining(item.end, now), style: TextStyle(color: accent, fontSize: 12, fontWeight: FontWeight.w800))],
          ]),
        );
      }),
    ]);
