...
class TopHeader extends StatelessWidget {
  ...
  String _subtitle(String language) {
    switch (language) {
      case 'bn': return 'আপনার দৈনন্দিন ইসলামিক সঙ্গী';
      case 'ar': return 'رفيقك الإسلامي اليومي';
      case 'en': default: return 'Your Daily Islamic Companion';
    }
  }

  String _assalamuAlaikum(String language) {
    switch (language) {
      case 'bn': return 'আসসালামু আলাইকুম';
      case 'ar': return 'السلام عليكم';
      case 'en': default: return 'Assalamu Alaikum';
    }
  }
  ...
  String _notificationTooltip(String language) {
    switch (language) {
      ...
      case 'en': default: return 'Notifications';
    }
  }
  ...
  String _profileTooltip(String language, bool signedIn) {
    if (signedIn) {
      switch (language) {
        ...
        case 'en': default: return 'Account';
      }
    }
    switch (language) {
      case 'bn': return 'লগইন';
      case 'ar': return 'تسجيل الدخول';
      case 'en': default: return 'Login';
    }
  }

  String _logoutLabel(String language) {
    switch (language) {
      case 'bn': return 'লগআউট';
      case 'ar': return 'تسجيل الخروج';
      case 'en': default: return 'Logout';
    }
  }
  ...
  Future<void> _openAccount(BuildContext context, User user) async {
    await showModalBottomSheet<void>(
      ...
      builder: (sheetContext) {
        ...
        final language = sheetContext.read<SettingsProvider>().languageCode;
        return SafeArea(
          child: Padding(
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    ...
                    label: Text(_logoutLabel(language)),
                  ),
                ),
              ],
            ),
  ...
  Widget _profileFallback(BuildContext context, User? user) {
    ...
    return Icon(signedIn ? Icons.person_rounded : Icons.person_outline_rounded, size: 24, color: theme.colorScheme.primary);
  }
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Row(
                          children: [
                            Flexible(child: Text('NurVerse', maxLines: 1, overflow: TextOverflow.ellipsis, style: theme.textTheme.titleLarge?.copyWith(fontWeight: FontWeight.bold))),
                            const SizedBox(width: 8),
                            Flexible(child: Text('نُورٌ عَلَىٰ نُورٍ', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontFamily: 'Amiri', fontSize: 12, color: theme.colorScheme.primary.withValues(alpha: 0.80), fontWeight: FontWeight.w600))),
                          ],
                        ),
                        const SizedBox(height: 0),
                        Text(subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: theme.textTheme.bodySmall?.copyWith(color: context.secondaryTextColor, fontSize: 14)),
                      ],
                    ),
                  ),
                  const SizedBox(width: 4),
                  SizedBox(width: 40, height: 40, child: IconButton(padding: EdgeInsets.zero, onPressed: onNotificationTap, tooltip: _notificationTooltip(language), icon: const Icon(Icons.notifications_none_rounded))),
                ...
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(assalamuAlaikum, style: theme.textTheme.bodyLarge?.copyWith(fontSize: 14, fontWeight: FontWeight.w700)),
                        const SizedBox(height: 0),
                        Text(localizedGreeting, style: theme.textTheme.bodyMedium?.copyWith(fontSize: 14, color: context.secondaryTextColor)),
                      ],
                    ),
                  ),
                  Container(
