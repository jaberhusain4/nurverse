...
class PrayerSchedule extends StatelessWidget {
  ...
  const PrayerSchedule({super.key, required this.controller, required this.isFriday});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: controller.prayers.map((prayer) {
        ...
        final String rawName = prayer['name']?.toString() ?? prayer['nameBn']?.toString() ?? prayer['nameAr']?.toString() ?? '';
...
class PrayerScheduleTile extends StatelessWidget {
  ...
  const PrayerScheduleTile({super.key, required this.name, required this.start, required this.end, required this.jamaat, required this.isCurrent, required this.isFriday});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isCurrent ? primary.withValues(alpha: .18) : primary.withValues(alpha: .06)),
      ),
      child: Row(
        children: [
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                if (isCurrent) ...[
                  ...
                  Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 2), decoration: BoxDecoration(color: primary, borderRadius: BorderRadius.circular(20)), child: Text(l10n.current, style: const TextStyle(color: Colors.white, fontSize: 8, fontWeight: FontWeight.bold))),
                ],
              ]),
              const SizedBox(height: 4),
              Text('$start - $end', style: theme.textTheme.bodySmall?.copyWith(color: context.secondaryTextColor)),
            ]),
          ),
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
            ...
            Text(l10n.jamaat, style: theme.textTheme.bodySmall?.copyWith(fontSize: 9, color: context.secondaryTextColor)),
          ]),
        ],
      ),
    );
